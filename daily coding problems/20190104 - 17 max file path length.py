...
def max_path_length(path):
   split = [(value.replace('\t', ''), value.count('\t')) for value in path.split('\n')]
   max_length = 0
   ...
   for index, value in enumerate(split):
       if '.' in value[0]:
           ...
           for index2 in range(index, -1, -1):
               if split[index2][1] == depth:
                   depth -= 1
                   path_stack.append(split[index2][0])

                   if depth == 0:
                       break

           path_stack.append(split[0][0])
           max_length = max(max_length, len('/'.join(reversed(path_stack))))

   return max_length


s = 'dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext'
assert max_path_length(s) == 32
...
s = 'dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2'
assert max_path_length(s) == 21

s = 'dir\n\tsubdir1\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2'
assert max_path_length(s) == 0